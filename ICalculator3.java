...
// ASSIGN. 6 === Inheritance Basics Problem 4
...
So you are to write the definition of a subclass ICalculator3 based on ICalculator The class ICalculator3 overrides the div method of ICalculator. It checks its argument and if the argument is zero it prints to standard output the message "ZERO DIVIDE ATTEMPTED" and simply returns the value of currentValue. Otherwise, if the argument is NOT zero, the method invokes the base class method div from ICalculator and returns the value that it receives from that call.
*/
public class ICalculator3 extends ICalculator {
    ...
    public int div(int value) {
        if (value == 0) {
            ...
            return getCurrentValue(); // Return the current value without division
        } else {
            return super.div(value); // Call the base class method for division
        }
    }
}
